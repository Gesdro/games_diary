...
export async function GET() {
	...
	for (let page = 1; page <= maxPages; page++) {
		const listUrl = `https://api.rawg.io/api/games?key=${RAWG_API_KEY}&page=${page}&page_size=${pageSize}`;
		...
		console.log(`Seite ${page}: ${validGames.length} Spiele importiert`);
	}

	if (allGames.length === 0) {
		return json({ status: 'error', message: 'Keine Spiele zum Einfügen!' }, { status: 400 });
	}
